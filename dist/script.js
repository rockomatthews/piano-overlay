...
let builtInKeyboard = new AudioKeys({
  rows: 2
});
...
let synthFilter = new Tone.Filter(300, 'lowpass').connect(new Tone.Gain(0.4).toMaster());
let synthConfig = {
  oscillator: {
    type: 'fattriangle'
  },
...
function getSeedIntervals(seed) {
  ...
  for (let i = 0; i < seed.length - 1; i++) {
    ...
    let measure = _.minBy(['8n', '4n'], subdiv => Math.abs(rawInterval - Tone.Time(subdiv).toSeconds()));
...
function detectChord(notes) {
  ...
  return Tonal.PcSet.modes(notes).map((mode, i) => {
...
function startSequenceGenerator(seed) {
  ...
  function generateNext() {
    ...
    if (generatedSequence.length < 10) {
      lastGenerationTask = rnn.continueSequence(seedSeq, 20, temperature, [chord]).then(genSeq => {
...
function updateChord({
  ...
  if (remove && _.some(currentSeed, {
    note: remove
  })) {
    _.remove(currentSeed, {
      note: remove
    });
  }
...
function generateDummySequence() {
  ...
  // cause jank later.
  return rnn.continueSequence(buildNoteSequence([{
    note: 60,
    time: Tone.now()
  }]), 20, temperature, ['Cm']);
}
let bufferLoadPromise = new Promise(res => Tone.Buffer.on('load', res));
Promise.all([bufferLoadPromise, rnn.initialize()]).then(generateDummySequence).then(() => {
